;(function ($) {
	...
	$.fn.slideshow.transitions = {
		...
		crossfade: function (e, next) {
			e.nextSlide.css({'z-index': 1, opacity: 0}).animate({ opacity: 1 }, e.speed, next);
			e.currentSlide.css('z-index', 0).fadeOut(e.speed, next);
		},

		slide: function (e, next) {
			var forward = (e.nextIndex > e.currentIndex);
			var width = e.currentSlide.outerWidth();

			e.currentSlide.css('left', 0);
			e.nextSlide.css('left', (forward ? width : -width));
			e.currentSlide.add(e.nextSlide).animate({ left: (forward ? '-=' : '+=') + width }, e.speed, function () {
				e.currentSlide.css('left', 0);
				next();
			});
		}
